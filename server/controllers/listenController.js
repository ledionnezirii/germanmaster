...
const calculateJaccardSimilarity = (text1, text2) => {
  ...
  const words1 = new Set(
    normalizedText1.split(/\s+/).filter((word) => word.length > 0),
  );
  const words2 = new Set(
    normalizedText2.split(/\s+/).filter((word) => word.length > 0),
  );
...
const checkAnswer = asyncHandler(async (req, res) => {
  ...
  let message;
  if (isCorrect) {
    message = "Shkëlqyeshëm! Kaluat me rezultat të shkëlqyer!";
  } else if (score >= 50) {
    message = "Përpjekje e mirë! Ishit afër, vazhdoni të praktikoni.";
  } else {
...
const markAsListened = asyncHandler(async (req, res) => {
  ...
  const alreadyListened = test.listenTestsPassed.some(
    (completion) => completion.user.toString() === req.user.id,
  );
  if (alreadyListened) {
...
const createTest = asyncHandler(async (req, res) => {
  ...
  res
    .status(201)
    .json(new ApiResponse(201, test, "Listening test created successfully"));
});
  ...
  const completedTestIds = allTestsWithCompletion
    .filter((test) =>
      test.listenTestsPassed.some(
        (completion) => completion.user.toString() === req.user.id.toString(),
      ),
    )
    .map((test) => test._id);
...
const getAllTestsWithCompletion = asyncHandler(async (req, res) => {
  ...
  const testsWithCompletion = tests.map((test) => ({
    ...test,
    isCompleted: test.listenTestsPassed.some(
      (completion) => completion.user.toString() === req.user.id.toString(),
    ),
    userScore:
      test.listenTestsPassed.find(
        (completion) => completion.user.toString() === req.user.id.toString(),
      )?.score || 0,
  }));
