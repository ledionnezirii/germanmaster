...
      setSuccess(true)
    } catch (err) {
      // err.response?.data?.message mund të jetë mesazh gabimi nga serveri
      setError(err.response?.data?.message || "Ndodhi një gabim gjatë rivendosjes së fjalëkalimit.")
    } finally {
      setLoading(false)
    }
  }
...
        <h2 className="text-2xl font-bold text-gray-900 mb-2 text-center font-sans">Vendosni një Fjalëkalim të Ri</h2>
        <p className="text-gray-600 text-sm mb-6 text-center font-sans">
          Ju lutemi krijoni një fjalëkalim të ri për llogarinë tuaj.
        </p>
...
          <input
            type="password"
            placeholder="Fjalëkalimi i Ri" // Përkthyer
            value={password}
...
          <input
            type="password"
            placeholder="Konfirmo Fjalëkalimin e Ri" // Përkthyer
...
          >
            {loading ? "Duke ruajtur..." : "Vendos Fjalëkalimin e Ri"} {/* Përkthyer */}
          </button>
        </form>
      </div>
    </div>
