...
export const dictionaryService = {
  ...
  getMyUnlockedWords: (params = {}) => api.get("/dictionary/unlocks/my", { params }),
}
...
export const ttsService = {
  getAudio: (testId, text, level) => api.post(`/tts/audio/${testId}`, { text, level }).then((res) => res.data.url),

  getDictionaryAudio: (wordId, text, level) =>
    api.post(`/tts/dictionary/${wordId}`, { text, level }).then((res) => res.data.url),

  getPhraseAudio: (phraseId, text, level) =>
    api.post(`/tts/phrase/${phraseId}`, { text, level }).then((res) => res.data.url),

  // Dialogue audio - single line
  getDialogueAudio: (dialogueId, lineIndex, text, level) =>
    api.post(`/tts/dialogue/${dialogueId}/${lineIndex}`, { text, level }).then((res) => res.data.url),

  getCategoryAudio: (categoryId, wordIndex, text, level) =>
    api.post(`/tts/category/${categoryId}/${wordIndex}`, { text, level }).then((res) => res.data.url),
